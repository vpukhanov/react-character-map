...
class CharacterMap extends React.Component {
  ...
  setupCharactersAtTab(tab) {
    ...
    const { charList, categoryList } = this.charListFromCharacters(
      characters,
      tab
    );
  ...
  componentDidMount() {
    ...
    if (
      false !== this.props.autofocus &&
      this.props.showFilter !== false &&
      this.searchInput &&
      "focus" in this.searchInput
    ) {
    ...
    Object.keys(characters).forEach((group) => {
      Object.keys(characters[group]).forEach((character) => {
        ...
        if (1 === search.length) {
          ...
          var index = characters[group][character].name
            .toLowerCase()
            .indexOf(search.toLowerCase());
          if (-1 !== index) {
  ...
  handleSearchChange(e) {
    ...
    if ("" === search) {
      ...
      var filteredCharacters = this.resultsCache[search]
        ? this.resultsCache[search]
  ...
  getCategoryName(category) {
    ...
    if (
      ...
      "string" !== typeof categoryNames[category]
    ) {
      return category;
    }
  ...
  charListFromCharacters(characters, active) {
    ...
    var charList = Object.keys(characters).map(function (category) {
      ...
      if (self.activeTab === i) {
        ...
        var currentItems = Object.keys(characters[category]).map(function (
          p,
          c
        ) {
          return (
            <li key={"topli" + p}>
              <button
                ...
                onClick={(e) =>
                  self.charClickHandler(e, characters[category][p])
                }
              >
      ...
      categoryList.push(
        <li
          ...
          className={
            ...
            (self.activeTab === i ? " active" : "")
          }
        >
          <button data-category-index={i} onClick={self.clickCategoryHandler}>
      ...
      return (
        <li key={"innerli" + category + i} data-category-name={category}>
          <ul
            className={
              "charMap--category " + (self.activeTab === i ? " active" : "")
            }
          >
            {currentItems}
          </ul>
  ...
  render() {
    ...
    const characterListLabelText =
      this.props.characterListLabelText || "Character List";
    const showFilter = this.props.showFilter !== false;
      ...
      <div className="charMap--container">
        ...
        {"" === search && (
          <ul
            ...
            aria-label={categoriesLabelText}
          >
            {categoryList}
          </ul>
        )}
